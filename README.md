...
툴바가 나타납니다: **왼쪽**에 시험 유형(모의고사/지필평가) 선택 버튼과
시간 기준 드롭다운, **오른쪽**에 테마 토글(해/달 아이콘)과 전체화면 아이콘.
설정 모달은 따로 없고, 모든 옵션이 툴바에 바로 노출되어 있습니다.
...
- `%APPDATA%\HyoExam\settings.json` — 테마·좌우 비율·시간 기준·마지막 선택
  시험 유형 (앱이 자동 생성/저장).
